...
- 两种题型交替：看名字选图 / 看图选名字
- 每轮正确答案声优绝不重复
- 实时计时、键盘 1/2/3/4 快捷作答、自动校验损坏图片
- 图片统一缩放白底填充（300x300）

### 网页版文件结构
```text
web_app.py              # Flask 后端（出题/判题/计时/结算/图片服务）
web/
  ├─ index.html         # 页面结构（选择页 / 答题页 / 弹窗）
  ├─ style.css          # 样式
...
### 运行网页版
> **目前已部署网址：**[站点✌](https://rdnsy.uminocloud.top) & [站点🐅](https://rdnsy.nagisa.mom)
```bash
